...
//TODO: Add transactions to this method
	...
	*/
	...
	/**Used to get the attribute value of a product in the selectable parts added to an order line. Example is fabric colour or chain length.
	 * 
	 * @param cOrderLineID
	 * @param attributeName
	 * @return
	 */
	public static String getAttributeLineProductInstance(int cOrderLineID, String attributeName) {
		StringBuffer sql = new StringBuffer	("SELECT mai.value FROM bld_line_productinstance bpsi ");
	...
	public static int getChainAttributeIDByLength(int drop) {
		...
		{
			double length = Integer.parseInt(attributeValue.getName());
			if(length >= targetLength && length < drop)
			{
...
private static ArrayList <Integer> processIDs(Properties pCtx, MBLDLineProductInstance[] instance, int parentMproduct_ID, boolean isSell) {
	 ...
	 * 
	 * @param fabricID
	 * @param headingWidthSwavePerCurtain
	 * @param trxName
	 * @return
	 */
