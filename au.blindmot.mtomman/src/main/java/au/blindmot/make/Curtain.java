...
public class Curtain extends RollerBlind {
	...
	public BigDecimal getCurtainFabricQty(int fabricID) {
		...
		if(isSwave) 
		{
			...
			else
			{
				...
				BigDecimal dropsPerCurtain = MtmUtils.getDropsPerCurtainSWave(fabricID, headingWidthPerCurtain.intValue(), trxName);
		...
		else
		{
			...
			else
			{
				...
				return dropsPerCurtain.multiply(fabricCutDrop).multiply(BigDecimal.valueOf(numOfCurtains));
				// .multiply(BigDecimal.valueOf(numOfCurtains) seems to be doubling actual needed qty.
			}
		}
	}
	...
	public enum CurtainConfig {
		...
		PART_TYPE_DRIVE_GEARBOX("Curtain drive gearbox");
	...
	public boolean addMtomItemDetail() {
		...
		//Add heading width
		double roundTo1 = 0.5;
		if(headingWidthPerCurtainField != null)
		{
