...
public class RollerBlind extends MadeToMeasureProduct{
	...
	private ArrayList<KeyNamePair> chain = new ArrayList<KeyNamePair>();
	...
	private static String CHAIN_LENGTH = "Chain length";
	...
	protected void populatePartTypes(int m_product_id) {
		 ...
		 */
			for(int i = 0; i < parts.length; i++) {
				...
				else if(partType.equalsIgnoreCase(CHAIN))
				{
					chain.add(new KeyNamePair(productIDs[i], names[i]));
				}
...
public boolean performOperationConditionSet(MBLDProductNonSelect mBLDPNonSelect) {
	...
	private int getAttributeSetInstanceIDChain () {
		StringBuffer sql = new StringBuffer("SELECT mai.m_attributesetinstance_id ");
		sql.append("FROM bld_line_productinstance bpsi ");
		sql.append("JOIN c_orderline col ON col.bld_line_productsetinstance_id = bpsi.bld_line_productsetinstance_id ");
		sql.append("JOIN m_attributesetinstance masi ON masi.m_attributesetinstance_id = bpsi.m_attributesetinstance_id ");
		sql.append("JOIN bld_line_productinstance bpi ON bpi.bld_line_productsetinstance_id = bpsi.bld_line_productsetinstance_id ");
		sql.append("JOIN m_attributeinstance mai ON mai.m_attributesetinstance_id = bpi.m_attributesetinstance_id ");
		sql.append("JOIN m_attribute ma ON ma.m_attribute_id = mai.m_attribute_id ");
		sql.append("WHERE col.c_orderline_id = ");	
		sql.append(mBLDMtomItemLine.getC_OrderLine_ID());
		sql.append(" AND ma.name = ");
		sql.append("'");
		sql.append(CHAIN_LENGTH);
		sql.append("'");
		sql.append(" LIMIT 1");
		
		int attributeSetInstanceID = DB.getSQLValue(null, sql.toString());	
		
		if (attributeSetInstanceID > 0) return attributeSetInstanceID;
		return 0;
	}
	
	private int getBldLineProductInstanceID(int mProductID, int mBldLineProductSetinstanceID, int bldProductParttypeID) {
		StringBuffer sql = new StringBuffer("SELECT bld_line_productinstance_id FROM bld_line_productinstance blp ");
		sql.append("WHERE blp.m_product_id = ");
		sql.append(mProductID);
		sql.append(" AND blp.bld_line_productsetinstance_id  = ");
		sql.append(mBldLineProductSetinstanceID);
		sql.append(" AND blp.bld_product_parttype_id = ");
		sql.append(bldProductParttypeID);
		int bldLineProductInstanceID = DB.getSQLValue(null, sql.toString());
		if(bldLineProductInstanceID > 0) return bldLineProductInstanceID;
		return 0;
	}
